...
class CartControllerTest extends BaseController {
    ...
    void shouldGetUserCard() throws Exception {
        when(itemService.findUserCard(USER_EMAIL)).thenReturn(ITEM_RESPONSE_DTO);
        mockMvc.perform(get("/cart/user@endava.com"))
                .andDo(print())
                .andExpect(status().isOk())
                .andExpect(content().contentType(MediaType.APPLICATION_JSON))
                .andExpect(content().json(createExpectedBody(ITEM_RESPONSE_DTO)));

        verify(itemService).findUserCard(USER_EMAIL);
    }
}